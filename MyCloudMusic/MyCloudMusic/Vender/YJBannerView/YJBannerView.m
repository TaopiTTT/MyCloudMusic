...
- (void)layoutSubviews{
    ...
    //计算指示器frame，这里只处理了系统指示器，后续制作新课程时争取换个框架，或者用自动布局重写一个
    if ([UIDevice currentDevice].systemVersion.doubleValue >= 14.0) {
        //iOS 14

        //不能写死宽高，要使用系统提供的API计算宽高
        ...
        //设置宽高
        frame.size.width = pageControlSize.width;
        frame.size.height = pageControlSize.height;

        //x
        frame.origin.x = (self.frame.size.width - frame.size.width)/2;

        //y
        frame.origin.y = self.collectionView.height_bannerView - pageControlSize.height;
